function [velocityMap,position_x, position_y,position_t,opt]=sticcs_batch(series1,series2,opt)
...
if ~isfield(opt, 'timeFrame'), opt.timeFrame = 15; end %time delay (s) between subsequent frames
if ~isfield(opt, 'timeDelay'), opt.timeDelay = 5; end %extra time delay (s) between two channels
if ~isfield(opt, 'tauLimit'), opt.tauLimit = 9; end % what is highest time lag to comute stics of TOI
%filtering: choose 'FourierWhole','MovingAverage','butterIIR','none'
if ~isfield(opt, 'filtering'), opt.filtering = 'butterIIR'; end 
...
if ~isfield(opt, 'axisTitle'), opt.axisTitle = 'Cross-Corr-21'; end %what will be used on vector map title
if ~isfield(opt, 'outputName'), opt.outputName = 'TestData'; end %output file name
...
for k=1:along_t % loop along time
    ...
    TOIFOV1=series1(:,:,(1+(k-1)/fracTOIshift*opt.TOIsize):(opt.TOIsize*(1+((k-1)/fracTOIshift))));
    TOIFOV2=series2(:,:,(1+(k-1)/fracTOIshift*opt.TOIsize):(opt.TOIsize*(1+((k-1)/fracTOIshift)))); 
