...
if ~isfield(opt, 'pixelSize'), opt.pixelSize = 0.03; end
if ~isfield(opt, 'timeFrame'), opt.timeFrame = 1; end %time delay (s) between subsequent frames
if ~isfield(opt, 'tauLimit'), opt.tauLimit = 21; end % what is highest time lag to compute stics of TO
%filtering: choose 'FourierWhole','MovingAverage','butterIIR','none'
if ~isfield(opt, 'filtering'), opt.filtering = 'MovingAverage'; end
...
if ~isfield(opt, 'omegaThreshold'), opt.omegaThreshold = 10; end % % how big (pixels) do you allow the radius of corr fn to be...removes wide corr fns...
...
if ~isfield(opt, 'axisTitle'), opt.axisTitle = 'F-actin Flow'; end %what will be used on vector map title
if ~isfield(opt, 'outputName'), opt.outputName = 'F-actin Flow'; end %output file name
if ~isfield(opt, 'path'), opt.path ='C:\Users\George\Desktop\STICCSPackage'; end
...
if ~isfield(opt, 'movieformat'), opt.movieformat ='avi'; end % movie format can be avi, jpeg or mp4
