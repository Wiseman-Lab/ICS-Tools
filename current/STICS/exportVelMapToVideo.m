function [velocityMap, opt] = exportVelMapToVideo(velocityMap,opt, m)
...
if strcmp(opt.bgImage, 'Other')
    try
        bg_series = readFileToStack(opt.bg_filePath{1,1});
    catch
        disp(['Could not find background file, please select it manually. ']);
        [fileID, filePath] = uigetfile("*.tif");
        bg_series = readFileToStack([filePath, fileID]);
    end
end

% opt.VelScaleRange = 'Global';
...
% ax = gca; 
[velocityMap, opt, fig, ax1, ax2] = plotSingleVectorMap(velocityMap, opt, m, k);
% [fig, ~, ~] = plotVelMap(velocityMap, opt, m, k);
% [fig, ~, ~, ~] = plotFlowAngles(velocityMap, opt, m, k, 0, 1, 0);
%     %colormapsize = length(overallgoodvectors);%
...
%         imagesc(series1(:, :, ceil(opt.position_t(i) - 0.5)));
%     elseif strcmpi(opt.bgImage, 'Other')
%         imagesc(bg_series(:, :, floor(opt.position_t(i) - 0.5)));
...
%       end
%           velocityMap{i}.autoScale = quiverc(gca, posyall(toigoodvect{i}),posxall(toigoodvect{i}),-vxall(i,toigoodvect{i}),-vyall(i,toigoodvect{i}),1);
...
%     end
    ...
    if strcmp(opt.exportimages,'y') && mod(k,opt.OutputEvery)==0
        if strcmp(opt.imagesformat,'png')
            ext='png';
            export_fig([opt.path 'VelocityMapTime_' opt.outputName '_' num2str(round(opt.position_t(k)*opt.timeFrame)) 's.' ext],gcf)
