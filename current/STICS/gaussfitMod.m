function [a, res, RESIDUAL,grid] = gaussfit(corr,type,pixelsize,whitenoise);
...
X0(EvilX0) = size(corr,2);
    ...
    % radius to zero
    lb = [0 0 0 -1 min(min(grid)) min(min(grid)) -pi/4]; %mod, added extra radius bound and rotation bound
    ub = [Inf Inf Inf Inf Inf Inf pi/4]; %mod
    
    weights = ones(size(corr));
             for x = 1:size(corr,1) %these loops remove NaN values from fitting
                for y = 1:size(corr,2)
                    if isnan(corr(x,y))
                        weights(x,y) = 0;
                        corr(x,y) = 0;
...
initguess = [g0 wguess y0 X0 Y0];
    ...
    elseif strcmp('2delliptical',type)
        for i=1:size(corr,3)
        a0 = initguess(i,:);
        a0xy(1:2) = initguess(i,1:2);
        a0xy(3) = a0xy(2);
        ...
        %better guesses
        a0xy(2:3) = [14,0.9];
        [a(i,:),res(i),RESIDUAL,EXITFLAG,OUTPUT,LAMBDA] = lsqcurvefit(@gauss2delliptical,a0xy,grid,corr(:,:,i).*weights(:,:,i),lb,ub,curvefitoptions,weights(:,:,i));%mod
        end
     elseif strcmp('2dellipticalsimp',type)
        for i=1:size(corr,3)
        a0 = initguess(i,:);
        a0xy(1:2) = initguess(i,1:2);
        a0xy(3) = a0xy(2);
        ...
        lb(7) = -10;
        ub(7) = 1;
        [a(i,:),res(i),RESIDUAL,EXITFLAG,OUTPUT,LAMBDA] = lsqcurvefit(@gauss2dellipticalsimp,a0xy,grid,corr(:,:,i).*weights(:,:,i),lb,ub,curvefitoptions,weights(:,:,i));%mod
        end
