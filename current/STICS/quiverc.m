function autoscale = quiverc(varargin)
...
%-------------------------------------------------------------
    ...
    beta = 0.43;  % Width of the base of the arrow head relative to the length
    autoscale = 0.2; % Autoscale if ~= 0 then scale by this.
    ...
    if nin<5 % quiver(u,v) or quiver(u,v,s)
      [msg,x,y,u,v] = xyzchk(varargin{2:3});
    else
      [msg,x,y,u,v] = xyzchk(varargin{2:5});
    end
    if ~isempty(msg), error(msg); end

    if nin==4 || nin==6 % quiver(u,v,s) or quiver(x,y,u,v,s)
    ...
    CC(end,:) = [1 1 1];
    CC(1,:) = [0 0 0];
    vrn=round(vr/max(vr(:))*size(colormap,1));

    % Define target axes
    if isa(varargin{1},'matlab.graphics.axis.Axes')
        ax = newplot(varargin{1});
    else
        disp('Not a graphic axes object');
        ax = newplot;
    end
    ...
    % Commented out by DK on Jan 29/07 to make vector plots copyable as
    % metafiles
    if ~isempty(ms) % Plot marker on base
      hu = x; hv = y;
      hold on
      h3 = plot(ax, hu(:),hv(:),[col ms]);
