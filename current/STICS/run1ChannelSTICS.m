...
[image_data1,rect] = serimcropold(image_data1);
...
[velocityMap,position_x, position_y,position_t,opt] = stics_vectormapping(image_data1,opt);
...
opt.thresholdV = Inf; %((sqrt((opt.ROIsize^2)/2))*opt.pixelSize/(opt.timeFrame*opt.tauLimit))*60; %um/min
opt.timerDisplay = 'LR'; % option for placing the timer in the movie/images
...
for i = 1:length(list)
    namefile = list(i).name; disp(namefile);
    image_data = rd_img16([opt.pathin namefile]);
