function quiverColor2D(x,y,u,v,varargin)
%quiverColor2D creates a 2D quiver plot and adds a color coding. The color coding is
...
if ~isequal(size(x),size(y),size(u),size(v))
    error('X,Y,U and V have to be arrays of the same size.');
end
...
% We assume that the colormap matrix contains the colors in its rows
switch scale_by_magnitude
    case 1 
