function [a, R2] = gaussfitold(corr,type,pixelsize,whitenoise,radius)
...
if testForNaN == 0,
    ...
    switch lower(type)
        case '2d'
            ...
            %Determining R2
            Data = gauss2dwxy(a, grid, weights);
            R2 = 0;
            RSS = 0;
            TSS = 0;
            ...
            for i = 1:size(Data,1);
                for j = 1:size(Data,2);
                    RSS = RSS + (corr(i,j,1) - Data(i,j,1)).^2;
                    TSS = TSS + (corr(i,j,1) - mean2(corr(:,:,1))).^2;
                end
            end
            R2 = 1- (RSS/TSS);
